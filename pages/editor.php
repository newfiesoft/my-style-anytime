...
function mysat_editor_page(): void {
	...
	register_setting('mysat_editor_group', 'mysat_editor_placeholder', array(
...
function mysat_get_css_file_content(): void {
	...
	if ( ! isset( $_POST['file'] ) ) {
		wp_die( esc_html__( 'Missing file parameter.', 'my-style-anytime' ) );
	}
...
function mysat_update_css_file_content(): void {
	...
	if ( ! isset( $_POST['content'] ) ) {
		wp_send_json_error( 'Missing content parameter.' );
	}
	...
	if (function_exists('wp_filesystem')) {
		...
		if ($wp_filesystem) {
			// Get raw content for CSS (don't sanitize CSS as it would break selectors)
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- CSS content needs to be preserved as-is
...
function mysat_render_editor_page(): void {
    ...
    <div class="license-container">
        ...
        if (
            ...
            || sanitize_text_field( wp_unslash( $_GET['generate_file'] ) ) !== $visitorFileName ||
            ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'generate_file_nonce' ) )
        {
