<?php
//includes/back/back_style_and_script.php
...
function mysat_plugin_code_editor_page_script(): void {
	...
	if (in_array($current_page, $allowed_pages)) {
		...
		wp_localize_script('codemirror-library', 'mysat_editor_script_vars', array(
			...
			'security' => wp_create_nonce('mysat_nonce_action'), // Correct usage
		));
	}
}
...
//// Load scripts only on the Customization page
function mysat_enqueue_customization_scripts( $hook_suffix ): void {
	// Check if we are on the Customization page: adjust 'my-style-anytime_page_mysat_custom_page' to match your actual page slug.

	// Get plugin dir url name
	$plugin_dir_url = my_style_anytime_directory_url();

	if ( $hook_suffix === 'my-style-anytime_page_mysat_custom_page' ) {
		...
		wp_enqueue_script('mysat-color-picker-js', $plugin_dir_url . 'assets/js/color-picker.js', array(), '1.0.0', true);

		// Enqueue media uploader scripts
		wp_enqueue_media();


		// Enqueue custom color picker & media uploader script
		wp_enqueue_script('mysat-media-uploader-js', $plugin_dir_url . 'assets/js/customization.js', array(), '1.0.0', true);
	}
}

add_action( 'admin_enqueue_scripts', 'mysat_enqueue_customization_scripts' );