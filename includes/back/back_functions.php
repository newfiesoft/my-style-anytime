<?php
//includes/back/back_functions.php

defined('ABSPATH') or exit();
/***
Load functions for the back side are displayed inside the admin panels. Example /wp-admin/
 ***/
...
function mysat_back_get_current_user_roles_style(): void {
	...
	if ($current_user !== null) {
		...
		foreach ($roles as $role) {
			...
			// Enqueue style directly since $role is already part of $roles
...
function mysat_back_get_handle_file_generation(): void {
	...
	if (isset($_GET['generate_file'], $_GET['_wpnonce']) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'generate_file_nonce')) {

		// Handle file generation logic here
		$role_slug = sanitize_text_field( wp_unslash( $_GET['generate_file'] ) );
